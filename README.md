# 従業員・給与情報管理システム (HR SaaSモックアップ)

## 概要
本プロジェクトは、バックオフィス系SaaS（HR・給与計算ドメイン）を想定した従業員情報の管理アプリケーションです。
単なるCRUD機能の実装にとどまらず、実際の税務・給与業務で求められる「過去データのエビデンス保持」や「データ不整合の防止」といった要件をDB設計に落とし込んでいます。

現在は、Webアプリケーションの基礎とデータベース操作の仕組みを深く理解するため、あえてピュアPHPで実装しています。
今後は実務を見据え、本システムをベースにした**Laravelフレームワークへのリプレイス**を実施予定です。

## 開発の目的
現職（自治体系の業務SE）の税務・基幹システム保守運用で培った「課税ドメイン知識」と「システム保守の視点」を、モダンなWeb開発技術（PHP/Docker）と融合させ、HR系SaaS企業のエンジニアとして即戦力に近い形で貢献できる基盤を作ることを目標としています。

## システム構成
Webサーバー層とデータ層を分離した、実務に近いアーキテクチャを採用しています。
* App Container: PHP 8.2 / Apache
* DB Container: MySQL 8.0

## 技術的判断
* **Dockerの採用:** 開発環境への依存バグを排除し、チーム開発における再現性を担保。
* **コンテナ間通信:** DockerのDNS機能を用いてサービス名 `db` で名前解決を実装。
* **文字コードの統一:** `utf8mb4` を採用し、将来的なマルチバイト文字の取り扱いにおける堅牢性を確保。  

## 本プロジェクトの技術的・業務的なこだわり

### 1. 業務ドメイン知識を反映したDB設計
HR・給与計算システムならではの要件を考慮し、以下の設計を採用しています。
* **論理削除の採用 (`employees.deleted_at`)**
  * 退職等で従業員データが不要になった場合でも、過年度の税額修正や過去の給与計算根拠を遡って確認（エビデンス確認）できるよう、物理削除ではなく論理削除を採用しています。
* **意図的なカスケード削除の非採用 (`salaries`テーブル)**
  * 従業員情報の変更によって、過去の確定した給与支給履歴（＝課税・徴収の根拠データ）が連動して消失するリスクを防ぐため、あえて `ON DELETE CASCADE` は設定しない堅牢な設計としています。

### 2. 再現性とチーム開発を見据えた環境構築・運用
* **Dockerの採用:** 開発環境への依存バグを排除し、チーム開発における環境構築の再現性を担保しています。（App: PHP 8.2/Apache, DB: MySQL 8.0）
* **Git (GitHub Flow) を用いた擬似チーム開発運用:** 常に `main` ブランチへ直接コミットするのではなく、機能ごとにブランチを切り、Pull Requestを作成してマージする、実務を想定したバージョン管理フローを実施しています。

### 3. セキュリティ対策
* **SQLインジェクション対策:** PDOのプリペアドステートメントを用い、悪意のあるSQL実行を防いでいます。
* **XSS対策:** 画面出力時に `htmlspecialchars` を用い、悪意のあるスクリプト実行を無害化しています。

## 今後の展望 (Laravelリプレイス計画)
本プロジェクトを基盤として、以下のステップでアーキテクチャの高度化と実務適応を進める計画です。

1. **Laravelフレームワークへの移行**
   - MVCアーキテクチャを採用し、関心の分離による保守性を高めます。
2. **Laravel Sailの導入と環境構築の効率化**
   - 現在はWeb技術の根底を理解するため、あえて自前で `docker-compose.yml` を定義し環境構築を行いました。
   - 次のステップでは、開発効率の最大化とチーム開発における環境の標準化を考慮し、Laravel公式の「Laravel Sail」を導入して開発環境を構築する計画です。
3. **DB管理の高度化**
   - マイグレーション機能によるスキーマ管理と、Eloquent ORM / SoftDeletes を活用した論理削除ロジックのフレームワーク標準への移行を行います。  

## 動作イメージ
### 一覧表示画面
![一覧表示](docs/images/list-view.png)

### 編集画面
![編集画面](docs/images/edit-form.png)

## 起動方法 (Quick Start)
1. GitHubからクローン
   ```bash
   git clone https://github.com/m-kazuha-dev/saas-project-01.git
   ```
2. Docker環境のビルドと起動
   ```bash
   docker-compose up -d --build
   ```
3. 動作確認  
   ブラウザで http://localhost:8080/ にアクセスしてください。

## システム構成/ディレクトリ構造
   ```plaintext
   saas-project-01/
   ├── docker/                 # インフラ環境設定 (PHP, MySQL)
   │   ├── mysql/init.sql      # DB初期化・DDL定義
   │   └── php/Dockerfile
   ├── src/                    # アプリケーションソースコード (ピュアPHP)
   └── docker-compose.yml
   ```

## 開発の記録 (CHANGELOG)
* **2026-07-01:** 論理削除の実装。退職等で従業員データが不要になった場合でも、過年度の税額修正や過去の給与計算根拠を遡って確認（エビデンス確認）できるよう、物理削除ではなく論理削除を採用。
* **2026-06-30:** 「Laravel Sail」の導入。開発環境構築における依存関係の複雑さを排除し、どの開発者環境でも「同一の環境」を即座に立ち上げられる再現性を担保するため採用。
* **2026-06-30:** Laravel移行開始。素のPHPで実装した際に課題となった「ルーティング・コントローラーの複雑化」を解消するため、MVCアーキテクチャの導入を決定。Eloquent ORMによる操作の簡略化、および標準搭載されたバリデーション・CSRF対策によるセキュリティの堅牢化を目的として導入。
* **2026-06-30:** ドキュメントの刷新。技術的判断の言語化と文字コード対策の経緯を記録。
* **2026-06-30:** 従業員編集機能の実装。PDOによるUPDATE SQLの実装とバリデーションの強化。
* **2026-06-29:** 文字化け対策の実施。データベースおよびアプリケーション層の文字コードを `utf8mb4` に統一。
* **2026-06-28:** 初期基盤構築。DockerによるPHP/MySQL環境の構築およびコンテナ間通信（名前解決）の実装。